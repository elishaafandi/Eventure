...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['approval_letter']) && $_FILES['approval_letter']['name'] != '') {
        ...
        if (move_uploaded_file($_FILES["approval_letter"]["tmp_name"], $target_file)) {
            // Store the uploaded file name in the database (if needed)
            // You can store this file name in your event record if needed
            // $update_file_query = "UPDATE event SET approval_letter = ? WHERE event_id = ?";
            // $stmt_update_file = $conn->prepare($update_file_query);
            // $stmt_update_file->bind_param("si", $approval_letter, $event_id);
